// Hook Handler - Handles hook telemetry, metrics, and policies
package main
...
func hookMetricsHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := queryRowWithTimeout(r.Context(), query, args...).Scan(
	...
	if err == nil {
		rows, err := queryWithTimeout(r.Context(), patternQuery, patternArgs...)
		if err != nil {
...
func hookLimitsHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := queryRowWithTimeout(r.Context(), overrideQuery, agentID, orgID, today).Scan(&overrideCount)
	if err != nil && err != sql.ErrNoRows {
	...
	baselineQuery := `SELECT COUNT(*) 
		...
		AND source = 'hook'
		AND DATE(created_at) >= $3`
	err = queryRowWithTimeout(r.Context(), baselineQuery, agentID, orgID, weekStart).Scan(&baselineCount)
	if err != nil && err != sql.ErrNoRows {
...
func reviewHookBaselineHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = queryRowWithTimeout(r.Context(), "SELECT id, reviewed, reviewed_by, reviewed_at FROM hook_baselines WHERE id = $1", baselineID).
